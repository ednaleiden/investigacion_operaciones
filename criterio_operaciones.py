...
class MatrixApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Matriz de Criterio")
        
        # Configurar estilo global
        style = ttk.Style(self.root)
        style.theme_use("arc")  # Puedes cambiar el tema ("arc", "plastik", "breeze", etc.)
        style.configure("TButton", font=("Arial", 12, "bold"), padding=6)
        style.configure("TLabel", font=("Arial", 10), padding=5)
        style.configure("TEntry", font=("Arial", 10))
        
        # Frame principal para agrupar la entrada de datos
        main_frame = ttk.Frame(self.root, padding="10 10 10 10")
        main_frame.grid(row=0, column=0, sticky="nsew")
        
        # Entrada para el número de filas y columnas
        ttk.Label(main_frame, text="Filas:").grid(row=0, column=0, padx=5, pady=5)
        self.entry_rows = ttk.Entry(main_frame, width=5)
        self.entry_rows.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Columnas:").grid(row=0, column=2, padx=5, pady=5)
        self.entry_cols = ttk.Entry(main_frame, width=5)
        self.entry_cols.grid(row=0, column=3, padx=5, pady=5)

        self.generate_button = ttk.Button(main_frame, text="Generar Matriz", command=self.generate_matrix)
        self.generate_button.grid(row=0, column=4, padx=10, pady=10)

        # Frame para la matriz y sus entradas
        self.matrix_frame = ttk.Frame(self.root, padding="10 10 10 10")
        self.matrix_frame.grid(row=2, column=0, columnspan=5, padx=5, pady=5)

        # Botones de operación (se ubican en la ventana principal)
        self.submit_button = ttk.Button(self.root, text="Mostrar Matriz", command=self.show_matrix, state="disabled")
        self.submit_button.grid(row=3, column=0, columnspan=5, pady=10)

        self.criteria_button = ttk.Button(self.root, text="Criterio valor esperado", command=self.calculate_criteria, state="disabled")
        self.criteria_button.grid(row=4, column=0, columnspan=5, pady=10)

        self.max_button = ttk.Button(self.root, text="C. Máxima Posibilidad", command=self.calculate_max_possibility, state="disabled")
        self.max_button.grid(row=5, column=0, columnspan=5, pady=10)

        self.lament_button = ttk.Button(self.root, text="C. Lamentaciones Mínimas", command=self.calculate_lamentaciones_minimas, state="disabled")
        self.lament_button.grid(row=6, column=0, columnspan=5, pady=10)

        self.varianza_button = ttk.Button(self.root, text="C. Mínima Varianza", command=self.calculate_minima_varianza, state="disabled")
        self.varianza_button.grid(row=7, column=0, columnspan=5, pady=10)

        self.resumen_button = ttk.Button(self.root, text="CUADRO RESUMEN", command=self.calculate_cuadro_resumen, state="disabled")
