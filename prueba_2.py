...
class MatrixApp:
    def __init__(self, root):
        ...
        self.criteria_button = ttk.Button(operations_frame, text="Criterio valor esperado", state="disabled", command=self.calculate_criteria)
        self.criteria_button.grid(row=0, column=1, padx=5, pady=5)
        
        self.max_button = ttk.Button(operations_frame, text="C. Máxima Posibilidad", state="disabled", command=self.calculate_max_possibility)
        self.max_button.grid(row=0, column=2, padx=5, pady=5)

        self.min_possibility_button = ttk.Button(operations_frame, text="C. Mínima Posibilidad", state="disabled", command=self.calculate_minima_possibility)
        self.min_possibility_button.grid(row=0, column=3, padx=5, pady=5)
        
        self.lament_button = ttk.Button(operations_frame, text="C. Lamentaciones Mínimas", state="disabled", command=self.calculate_lamentaciones_minimas)
        self.lament_button.grid(row=0, column=4, padx=5, pady=5)
        
        self.varianza_button = ttk.Button(operations_frame, text="C. Mínima Varianza", state="disabled", command=self.calculate_minima_varianza)
        self.varianza_button.grid(row=0, column=5, padx=5, pady=5)
        
        self.resumen_button = ttk.Button(operations_frame, text="Cuadro Resumen maximo", state="disabled", command=self.calculate_cuadro_resumen)
        self.resumen_button.grid(row=0, column=6, padx=5, pady=5)
    
        # Nuevo botón: Cuadro Resumen Mínimo
    ...
    def calculate_lamentaciones_minimas(self):
        ...
        frame1 = ttk.Frame(result_window, padding="10 10 10 10")
        ...
        frame2 = ttk.Frame(result_window, padding="10 10 10 10")
    ...
    def calculate_minima_varianza(self):
        ...
        label1.pack()
        tree1 = ttk.Treeview(frame1, columns=("Alternativa", "Criterio"), show="headings", height=self.rows)
        tree1.heading("Alternativa", text="Alternativa", anchor="center")
    ...
    def calculate_minima_possibility(self):
        ...
        # Buscar la probabilidad mínima y su columna asociada
        min_prob = min(probabilities)
        min_prob_index = probabilities.index(min_prob)

        # En esa columna, buscar el valor mínimo
        min_value = None
        min_row_index = None
        for i in range(self.rows):
    ...
    def calculate_cuadro_resumen_minimo(self):
        ...
        # Armar la tabla resumen (cada fila: [Alternativa, ranking_C1, ranking_C2, ranking_C3, ranking_C4])
        summary = []
        for i in range(self.rows):
            summary.append([row_titles[i], ranking_C1[i], ranking_C2[i], ranking_C3[i], ranking_C4[i]])

        # Contar la cantidad de “1” en cada fila (solo en aquellos criterios donde se asigna número)
        ones_count = []
        for i in range(self.rows):
            count = 0
            if ranking_C1[i] == 1:
        ...
        max_ones = max(ones_count)
        best_indices = [i for i, cnt in enumerate(ones_count) if cnt == max_ones]
        # En caso de empate, se puede usar otro criterio (por ejemplo, la suma de rangos numéricos, considerando "-" como un número alto)
        ...
        # Mostrar la tabla resumen y la mejor opción
        result_window = tk.Toplevel(self.root)
